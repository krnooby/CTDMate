...
PDF_SPLIT_MODE = "page"        # PDF는 페이지 단위 파싱 후, 헤딩 기반 청크
RAG_MAX_CHARS = 1600           # PDF 헤딩-세그먼트 2차 분할 길이
RAG_OVERLAP = 200
    ...
    for raw in inputs:
        try:
            ...
            if src.suffix.lower() == ".xlsx":
                # (A) 엑셀: JSONL은 "시트 1청크" 고정
                rag_items = xlsx_to_sheet_chunks(str(src))
                # MD는 Upstage 파서로 페이지 단위 파싱한 뒤 합본(원형 보존)
...
# === LangGraph/LLM용 문서 파서 래퍼 ===
from typing import List, Dict, Any
import json
from pathlib import Path

def _collect_combined_md(results: List[Dict[str, Any]]) -> str:
    parts = []
    for r in results or []:
...
def parse_documents(file_paths: List[str] | None = None, texts: List[str] | None = None) -> str:
    """
    LangGraph Tool용. 입력 파일을 Upstage 파서로 MD화하고 합쳐서 반환.
    반환: JSON 문자열 {documents, combined_markdown, meta}
    """
    file_paths = file_paths or []
    texts = texts or []

    res: Dict[str, Any] = {
        "documents": [{"path": p} for p in file_paths],
        "combined_markdown": "",
        "meta": {},
    }
    ...
    if file_paths:
        out = run(file_paths)  # 기존 파이프라인 실행
        res["meta"] = out
        combined = _collect_combined_md(out.get("results", []))
    ...
    res["combined_markdown"] = combined or ""
    return json.dumps(res, ensure_ascii=False)

# (선택) LangChain tool 데코레이터가 필요하면 아래 사용
try:
...
if _lc_tool:
    @_lc_tool("parse_documents")
    def parse_documents_tool(file_paths: List[str] | None = None, texts: List[str] | None = None) -> str:
        return parse_documents(file_paths=file_paths, texts=texts)
