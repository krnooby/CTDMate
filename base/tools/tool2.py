...
# === LangGraph/LLM용 규정 검증 래퍼 ===
import json

def verify_ctd(documents_json: str, default_section: str = "M2.6", auto_fix: bool = False) -> str:
    """
    LangGraph Tool용. parse_documents 출력(JSON 문자열)을 입력으로 받아
    RegulationRAGTool로 검증 후 VERIFY_SCHEMA 형태의 JSON 문자열을 반환.
    반환: {"status","missing_fields","hits","ref_hits"}
    """
    try:
        obj = json.loads(documents_json) if isinstance(documents_json, str) else (documents_json or {})
    except Exception:
        obj = {}
    content = str(obj.get("combined_markdown", "") or "")
    section = default_section

    tool = RegulationRAGTool(auto_normalize=auto_fix, enable_rag=True)
    result = tool.validate_and_normalize(section=section, content=content, auto_fix=auto_fix)
    ...
    out = {
        "status": "ok",
        "missing_fields": [],   # 필요 시 규칙 기반 누락 검출 로직 추가
        "hits": hits,
        "ref_hits": {},         # 2차 검색 결과가 있으면 채우기
    }
    return json.dumps(out, ensure_ascii=False)

# (선택) LangChain tool 데코레이터
try:
    from langchain_core.tools import tool as _lc_tool  # 환경에 맞춰 경로 조정
except Exception:
    _lc_tool = None

if _lc_tool:
    @_lc_tool("verify_ctd")
    def verify_ctd_tool(documents_json: str, default_section: str = "M2.6", auto_fix: bool = False) -> str:
        return verify_ctd(documents_json, default_section=default_section, auto_fix=auto_fix)
